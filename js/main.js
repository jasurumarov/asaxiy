...
const likedProducts = document.querySelector(".liked-section__liked-products")
...
// ADDER LIKED CARDS TO LIKED SECTION

function addToLikedSection(event) {
    if (event.target.name === "btn-heart") {
        let productCard = event.target.closest(".products-section__products__card");
        if (productCard) {
            let likedCard = productCard.cloneNode(true);
            likedCard.querySelector(".products__card-absolutes--heart").addEventListener("click", removeFromLikedSection);
            likedProducts.appendChild(likedCard);
        }
    }
}

products.addEventListener("click", addToLikedSection);

function removeFromLikedSection(event) {
    let likedCard = event.target.closest(".products-section__products__card");
    if (likedCard) {
        likedCard.remove();
    }
}

// LIKED SECTION DISPLAY BLOCK OR NONE

const headerLikedBtn = document.querySelector(".header-top__list__heart-btn")
const likedSection = document.querySelector(".liked-section")
const closeLikedSection = document.querySelector(".liked-section__close")

headerLikedBtn.addEventListener("click", () => {
    likedSection.classList.toggle("showLiked-section");
});
closeLikedSection.addEventListener("click", ()=>{
    likedSection.classList.remove("showLiked-section")
})